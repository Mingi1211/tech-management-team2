...
| 파일 | 내용 | 상태 |
|---|---|---|
| `seed-robot-courses.sql` | **로봇학부 35과목** (1학기 18 · 2학기 17) | 2026-09-19 입력 |
| — | 소프트웨어학부 | 미작성 |
| — | 정보융합학부 | 미작성 |

`schema.sql` 을 Run 한 뒤 이어서 Run 한다. **여러 번 돌려도 안전하다** —
같은 과목이면 덮어쓰고, 새 과목만 추가된다.

두 가지를 기억할 것.

- **`course.name` 이 정본이다.** W3 부터 입력 폼의 과목 선택 목록이 이 값을 그대로
  보여준다. 여기서 `자동제어1` 을 `자동제어I` 로 바꾸면 선배가 고를 이름 자체가 바뀐다
- **교수 연락처는 레포에 올리지 않는다.** 담당 교수명만 `note` 에 적었다

아직 못 채운 칸은 파일 맨 아래 확인 쿼리 2번으로 바로 뽑을 수 있다
(현재 로봇학실험 1·2·4 — 개설학기만 확인됐고 학수번호·학점·학년이 비어 있다).

## 다음

- **DB 담당 2명** — 소프트웨어·정보융합 교과과정표를 같은 형식의 seed 파일로
  (150과목 목표), 이어서 `prerequisite` · `track_course` 매핑
- **DB 담당 2명** — 로봇학실험 1·2·4 의 학수번호·학점·개설학년 채우기
