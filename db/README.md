...
전공필수라 전원이 듣고, 네 과목 모두 운영이 같고 분반이 달라도 같다 — 프로젝트 15주차
(기말 대체) · 중간 필기 1회 · 과제는 시험 주차 뺀 매주 · 퀴즈 없음. 선배 응답을 기다릴
이유가 없고, 폼으로 받으면 분반 수만큼 같은 내용이 중복으로 쌓인다.
`source = 'seed'` 로 들어가 있고 `course_id` 까지 연결돼 있다.

한 과목당 **부담 이벤트 14건** (과제 12 · 중간 1 · 프로젝트 1) 이 트리거로 생긴다.

아직 못 채운 칸은 `seed-robot-courses.sql` 맨 아래 확인 쿼리 3번으로 뽑는다
(현재 로봇학실험 1·2·4 의 학수번호와 담당 교수).

## 다음

- **DB 담당 2명** — 소프트웨어·정보융합 교과과정표를 같은 형식의 seed 파일로
  (150과목 목표), 이어서 `prerequisite` · `track_course` 매핑
- **DB 담당 2명** — 로봇학실험 1·2·4 의 학수번호·담당교수 채우기 (개설학년은 추정값)
